...
menu(where=@(this.count == 0 && isw11) type='taskbar' image=icon.settings expanded=true)
{
	menu(title="Apps" image=\uE254)
	{
        ...
        item(title='Putty' image cmd='C:\Program Files (x86)\PuTTY\PUTTY.exe')
		item(title='WinSCP' image cmd='C:\Program Files (x86)\WinSCP\WinSCP.exe' dir)
	...
	item(title="Process Explorer" image cmd='D:\PROGRAMMI\scoop\apps\sysinternals\current\procexp.exe')		
